...
> **Edge Function `membership-notify`** (code écrit, **non déployée**) : appelée par le trigger de l'entrée ci-dessus, lit `{membership_season_id, member_id}` et envoie via Resend un e-mail à l'adhérent (récapitulatif du dossier) et un e-mail au bureau (boîte club `narbo.nordik.club@gmail.com` + super-admins, dossier complet hors notes, décision du 2026-08-17 : ne part plus à tous les coachs). **Déploiement `supabase functions deploy membership-notify` à exécuter avant le tout premier insert dans `membership_seasons` en prod**, sinon 404 silencieux côté trigger (cf. entrée 40 ci-dessus). **Secret requis : `RESEND_API_KEY`**, déjà posé pour `send-notification-email`/`weekly-digest`/`daily-session-digest` — à revérifier avec `supabase secrets list` avant ce déploiement plutôt que de le supposer présent. **Test en prod** : insérer une ligne de test dans `membership_seasons` (avec un `members.email` de test), puis lire `select * from net._http_response order by created desc limit 5;` pour confirmer un `status_code` 200 et vérifier la réception des 2 e-mails.
